"""AI Agent Security — Phase E5 (N32 + per-model template routing → 90+ aim).
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None = None) -> list[AttackCandidate]:
        ...
        if use_split and has_time():
            # Dual-probe: plain vs inj_done raw/sec (robust host vs local latency).
            inj_idx = 5  # inj_done
            ...
            for _ in range(classify_reps):
                if not has_time():
                    break
                trial(inj_idx, probe_index)
                probe_index += 1
            plain_rps = _raw_per_second(latencies[FALLBACK_INDEX], raw_scores[FALLBACK_INDEX])
            inj_rps = _raw_per_second(latencies[inj_idx], raw_scores[inj_idx])
            for ti in (FALLBACK_INDEX, inj_idx):
                latencies[ti].clear()
                fires[ti] = 0
                raw_scores[ti] = 0
            prefer_slow = inj_rps > plain_rps * 1.15
            if prefer_slow or (slowest >= split_thr and inj_rps >= plain_rps * 0.9):
